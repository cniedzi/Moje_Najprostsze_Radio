...
void setup() {
  ...
  tft.setTextColor(ST77XX_CYAN); tft.setCursor(0,70); tft.print(Lista_Stacji[g_Stacja][0]); // Zmieniamy kolor czcionki na cyjanowy i wyświetlamy zdefiniowaną wcześniej nazwę stacji na pozycji (0, 70)
...
void loop() {
  ...
  if (result_R == DIR_CW && g_Stacja < 4) { // Jeżeli obrót zgodnie z ruchem wskazówek zegara i jeżeli aktualny indeks stacji jest mniejszy niż maksymalny indeks stacji - u nas 4, to
    ...
    tft.setTextColor(ST77XX_CYAN); tft.setCursor(0,70); tft.print(Lista_Stacji[g_Stacja][0]);  // Wyświetl nazwę wybranej stacji na pozycji (0, 70)
