...
void my_audio_info(Audio::msg_t m) {
  ...
  if (m.e == Audio::evt_streamtitle) {
    ...
    tytulStreamu = PL(tytulStreamu); // zamieniamy polskie znaki UTF-8 na polskie znaki w biblitece Adafruit GFX
    ...
    tft.print(tytulStreamu); //i wyświetlamy nowy tytuł streamu
  }
...
void wyswietlLogoStacji(String nazwaStacji) { // do funkcji przekazujemy nazwę stacji, dla której chcemy wyświetlić logo
  ...
  else tft.drawRGBBitmap(197, 45, RADIO, 67, 60);
}
// --------------------- L O G O -----------------------//
...
// ----------------------- P L -------------------------//
// Zamiana polskich liter UTF-8 na polskie znaki biblioteki Adafruit GFX
String PL(String _tekst) {
  _tekst.replace("ż", "\x90");
  _tekst.replace("ó", "\x8e");
  _tekst.replace("ł", "\x8c");
  _tekst.replace("ć", "\x8a");
  _tekst.replace("ę", "\x8b");
  _tekst.replace("ś", "\x8f");
  _tekst.replace("ą", "\x89");
  _tekst.replace("ź", "\x91");
  _tekst.replace("ń", "\x8d");
  _tekst.replace("Ż", "\x87");
  _tekst.replace("Ó", "\x85");
  _tekst.replace("Ł", "\x83");
  _tekst.replace("Ć", "\x81");
  _tekst.replace("Ę", "\x82");
  _tekst.replace("Ś", "\x86");
  _tekst.replace("Ą", "\x80");
  _tekst.replace("Ź", "\x88");
  _tekst.replace("Ń", "\x84");
  return _tekst;
}
// ----------------------- P L -------------------------//
...
void setup() {
  ...
  tft.print("NAJPROSTSZE"); // Wyswietlamy napis "NAJPROSTSZE" na pozycji (61, 0)
  tft.setCursor(7,30); // Ustawiamy pozycję (x, y) tekstu, na której będzie wyswietlony tekst
  tft.print("RADIO INTERNETOWE"); // Wyswietlamy napis "RADIO INTERNETOWE" na pozycji (7, 30)
  ...
  tft.setTextSize(3); // Ustawiamy rozmiar czcionki wyświetlacza na 3
  tft.setTextColor(TFT_ORANGE); // Ustawiamy kolor czcionki wyświetlacza
  tft.setCursor(5,0); // Ustawiamy pozycję (x, y) tekstu, na której będzie wyswietlony tekst
  tft.print("RADIO INTERNETOWE"); // Wyswietlamy napis "RADIO INTERNETOWE" na pozycji (5, 0)
  tft.setTextSize(2); // Ustawiamy rozmiar czcionki wyświetlacza na 2
  tft.setTextColor(TFT_MAGENTA); tft.setCursor(0, STACJE_POS_Y); tft.print(PL(String(Lista_Stacji[g_Stacja][0]))); // Zmieniamy kolor czcionki na cyjanowy i wyświetlamy zdefiniowaną wcześniej nazwę stacji
                                                                                                                   // wielkimi literami, na pozycji (0, STACJE_POS_Y). Przed wyświetleniem, konwertujemy Polskie
                                                                                                                   // znaki UTF-8 na polskie znaki w bibliotece Adafruit GFX.
...
void loop() {
  ...
  if (digitalRead(buttonEncoderL) == LOW) { // Jeżeli przycisk lewego enkodera jest wciśnięty, to...
    while (digitalRead(buttonEncoderL) == LOW){ // dopóki będzie on wciśnięty, 
      ...
      vTaskDelay(1); // Opóżnienie 1ms zapobiegająca ew. błądom watchdog, skutkującym nieplanowanymi restarami
    }
    ...
    if (g_Mute) { // Jeżeli aktywny jest tryb Mute, to
      ...
      tft.setTextSize(2); // Zmieniamy rozmiar czcionki na 2
      tft.setTextColor(TFT_WHITE, TFT_RED); tft.setCursor(0, 225); tft.print("Vol:"); // Ustawiamy kolor czcionki biały na czerwonym tle, ustawiamy kursor na pozycji (0, 225)
                                                                                      // i wyświetlamy napis "Vol:"
      tft.setCursor(48, 225); tft.printf("%02d", g_Volume); // Ustawiamy kursor na pozycji (48, 225) i wyświetlamy aktualną wartość ustawionej głośności,
                                                            // mimo że jesteśmy w trybie Mute, co sygnalizowane jest czerwonym tłem czcionki.
    }
      ...
      tft.setTextSize(2); // Zmieniamy rozmiar czcionki na 2
      tft.setTextColor(VOLUME_KOLOR, TFT_BLACK); tft.setCursor(0, 225); tft.print("Vol:"); // Ustawiamy kolor czcionki na VOLUME_KOLOR na czarnym tle, ustawiamy kursor na pozycji (0, 225)
                                                                                           // i wyświetlamy napis "Vol.:"
      tft.setCursor(48, 225); tft.printf("%02d", g_Volume); // Ustawiamy kursor na pozycji (48, 225) i wyświetlamy aktualną wartość ustawionej głośności.
    }
  }
  ...
  if (result_R == DIR_CW && g_Stacja < 4) { // Jeżeli obrót zgodnie z ruchem wskazówek zegara i jeżeli aktualny indeks stacji jest mniejszy niż maksymalny indeks stacji - u nas 4, to
    ...
    tft.setTextColor(TFT_MAGENTA); tft.setCursor(0,STACJE_POS_Y); tft.print(PL(String(Lista_Stacji[g_Stacja][0])));  // Wyświetl nazwę wybranej stacji na pozycji (0, 65) w kolorze magenta. Przed wyświetleniem, konwertujemy Polskie
                                                                                                                     // znaki UTF-8 na polskie znaki w bibliotece Adafruit GFX.
